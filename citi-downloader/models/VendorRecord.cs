...
namespace TrainingDownloader.models
{
    public class VendorRecord
    {
        ...
        public string LearnerWebCourseId { get; set; }
        ...
        {
            if (this.StageNumber.HasValue && this.StageNumber != 1)
            {
                return string.Format("{0}-{1}", this.VendorCourseId, this.StageNumber);
            }
            return this.VendorCourseId;
        }

        public bool IsComplete => (this.Score.HasValue && this.CompletionDate.HasValue) ? (this.Score >= this.PassingScore) : false;
        ...
        public DateTime? GetNullableCompletionDate()
        {
            return FormatNullableDateAsNullable(this.CompletionDate);
        }
        ...
        public DateTime? GetNullableRegistrationDate()
        {
            return FormatNullableDateAsNullable(this.RegistrationDate);
        }
        ...
        public DateTime? GetNullableExpirationDate()
        {
            return FormatNullableDateAsNullable(this.ExpirationDate);
        }
        ...
        protected DateTime FormatNullableDate(DateTime? nullableDateTime)
        {
        ...
        protected DateTime? FormatNullableDateAsNullable(DateTime? nullableDateTime)
        {
            if (!nullableDateTime.HasValue)
            {
                return null;
        ...
        {
            return (string.Format("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td><td>{8}</td><td>{9}</td><td>{10}</td><td>{11}</td><td><a href='{12}courses/details/{13}'>View</a></td></tr>",
                        this.VendorUserId, this.FirstName, this.LastName, this.EmailAddress, this.NetId, this.RegistrationDate, this.VendorCourseName, this.GetVendorCourseId(), this.Score, this.CompletionDate, this.ExpirationDate, this.InstitutionalEmailAddress, url, this.ID));
        }
        ...
        public Action<TableRowTag> ToTableRow()
        {
            return row =>
            {
                ...
                row.Cell(this.VendorCourseName);
                row.Cell(this.GetVendorCourseId());
                row.Cell(GetScore());
